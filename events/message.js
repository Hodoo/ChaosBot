...
*/
  ...
  if(message.content.startsWith(server.prefix + "say")) {
    ...
    if (message.content.split(" ", 2)[1].startsWith("<#")) {
      let channel = message.guild.channels.find('id', client.regex.channel.exec(message.content.split(" ", 2)[1])[1]);
      if (!channel) {
  ...
  if(message.content.startsWith(server.prefix + "version")) {
    let cmd = "version";
    message.channel.send(`Currently running ChaosBot v${client.version}`).catch(logSendError);
    return;
  }
  ...
  if(message.content.startsWith(server.prefix + "auth")) {
    ...
    if (arg.startsWith("<@")) {
      let target = client.regex.user.exec(arg)[1];
  ...
  if(message.content.startsWith(server.prefix + "welcome")) {
    ...
    if (args[1].startsWith("<#")) {
      let target = client.regex.channel.exec(args[1])[1];
