...
client.on('raw', packet => {
    ...
    if (['MESSAGE_REACTION_ADD', 'MESSAGE_REACTION_REMOVE'].includes(packet.t)) {
      ...
      channel.messages.fetch(packet.d.message_id).then(message => {
          ...
          if (messageReaction) messageReaction.users.set(packet.d.user_id, client.users.cache.get(packet.d.user_id));
          // Check which type of event it is before emitting
          if (packet.t === 'MESSAGE_REACTION_ADD') {
              client.emit('messageReactionAdd', messageReaction, client.users.cache.get(packet.d.user_id));
          }
          if (packet.t === 'MESSAGE_REACTION_REMOVE') {
              client.emit('messageReactionRemove', messageReaction, client.users.cache.get(packet.d.user_id));
          }
      });
